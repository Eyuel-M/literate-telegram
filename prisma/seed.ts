...
async function main() {
  ...
  const clientNova = await prisma.client.create({
    data: { name: "Nova Labs",       email: "hello@novalabs.io",      company: "Nova Labs Inc.",           color: "#7c3aed", status: "ACTIVE", workspaceId: workspace.id },
  });
  const clientPeach = await prisma.client.create({
    data: { name: "Peach & Co",      email: "studio@peachandco.com",  company: "Peach & Co Interiors",     color: "#f59e0b", status: "ACTIVE", workspaceId: workspace.id },
  });
  const clientMeridian = await prisma.client.create({
    data: { name: "Meridian Finance", email: "brand@meridianfin.com", company: "Meridian Financial Group", color: "#10b981", status: "ACTIVE", workspaceId: workspace.id },
  });
  const clientVolt = await prisma.client.create({
    data: { name: "Volt Digital",    email: "hi@voltdigital.co",      company: "Volt Digital Agency",      color: "#ef4444", status: "ACTIVE", workspaceId: workspace.id },
  });
  ...
  await prisma.deliverable.create({ data: { name: "Business Cards",   type: "BUSINESS_CARD",  status: "PENDING",     sortOrder: 2, projectId: pNovaBrand.id  } });
  await prisma.deliverable.create({ data: { name: "Social Media Kit", type: "SOCIAL_MEDIA",   status: "PENDING",     sortOrder: 3, projectId: pNovaBrand.id  } });
  const dLogoApproved = await prisma.deliverable.create({
    data: { name: "Updated Logomark", type: "LOGO",           status: "APPROVED",    sortOrder: 0, projectId: pPeachBrand.id },
  });
  await prisma.deliverable.create({ data: { name: "Color & Type Guide", type: "BRAND_IDENTITY", status: "IN_REVIEW", sortOrder: 1, projectId: pPeachBrand.id } });
  const dMeridianLogo  = await prisma.deliverable.create({ data: { name: "Logotype",       type: "LOGO",           status: "APPROVED", sortOrder: 0, projectId: pMeridianId.id   } });
  const dMeridianCards = await prisma.deliverable.create({ data: { name: "Business Cards", type: "BUSINESS_CARD",  status: "APPROVED", sortOrder: 1, projectId: pMeridianId.id   } });
  const dMeridianGuide = await prisma.deliverable.create({ data: { name: "Brand Guidelines", type: "BRAND_IDENTITY", status: "APPROVED", sortOrder: 2, projectId: pMeridianId.id } });
  const dVoltBanner    = await prisma.deliverable.create({ data: { name: "Campaign Banners", type: "SOCIAL_MEDIA",   status: "IN_PROGRESS", sortOrder: 0, projectId: pVoltCampaign.id } });
  await prisma.deliverable.create({ data: { name: "Social Templates", type: "SOCIAL_MEDIA", status: "PENDING", sortOrder: 1, projectId: pVoltCampaign.id } });

  // ── Versions ─────────────────────────────────────────────────
  const v1 = await prisma.version.create({ data: { number: 1, notes: "Initial concept exploration — three directions presented.", status: "APPROVED", deliverableId: dLogo.id } });
  const v2 = await prisma.version.create({ data: { number: 2, notes: "Refined direction B based on client feedback.",             status: "APPROVED", deliverableId: dLogo.id } });
  const v3 = await prisma.version.create({ data: { number: 3, notes: "Final adjustments — color variants and reversed versions.", status: "SENT",     deliverableId: dLogo.id } });
  const peachV1 = await prisma.version.create({ data: { number: 1, notes: "Subtle geometric update — retains warmth but feels more refined.", status: "APPROVED", deliverableId: dLogoApproved.id } });
  ...
  // ── Feedback ─────────────────────────────────────────────────
  const fb1 = await prisma.feedback.create({ data: { content: "Love concept B! The geometric shape feels very aligned with our tech-forward brand. Can we see it with a heavier weight?", author: "Maya Chen (Nova Labs)", resolved: true,  versionId: v1.id } });
  await prisma.feedbackReply.create({ data: { content: "Absolutely, I'll bring the weight up and explore how it reads at small sizes. Expect v2 by Friday.", author: "Eyuel Mulat", feedbackId: fb1.id } });
  const fb2 = await prisma.feedback.create({ data: { content: "The spacing feels much better. Can the wordmark have a bit more breathing room from the icon?", author: "Maya Chen (Nova Labs)", resolved: true,  versionId: v2.id } });
  await prisma.feedbackReply.create({ data: { content: "Good catch. Adjusted the lockup with 16px separation — feels much cleaner.", author: "Eyuel Mulat", feedbackId: fb2.id } });
  await prisma.feedback.create({ data: { content: "This looks great! One final request — can we get a version on a dark navy background?", author: "Maya Chen (Nova Labs)", resolved: false, versionId: v3.id } });
  await prisma.feedback.create({ data: { content: "This is exactly what we were looking for! Approved!", author: "Sophie T. (Peach & Co)", resolved: true, versionId: peachV1.id } });
  ...
  // ── Preset Templates (global, workspaceId = null) ─────────────
  const presets = [
    {
  ...
  for (const preset of presets) {
    await prisma.template.create({
      data: {
        ...
        isPreset:    true,
        workspaceId: null, // global preset — visible to all workspaces
  ...
  console.log("   Workspace   : Forma Studio (forma-studio)");

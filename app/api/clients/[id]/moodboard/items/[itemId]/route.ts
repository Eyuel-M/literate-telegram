...
async function canAccessItem(itemId: string, userId: string, workspaceId: string, role: string) {
  ...
  if (role === "ADMIN" && client.workspaceId === workspaceId) return item;
...
export async function PUT(req: NextRequest, { params }: { params: { id: string; itemId: string } }) {
  ...
  const item = await canAccessItem(params.itemId, userId, workspaceId, role);
